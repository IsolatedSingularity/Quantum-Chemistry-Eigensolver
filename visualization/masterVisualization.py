...
def animate_h2_dissociation(save_path):
    ...
    try:
        ...
        def animate(i):
            ...
            vertical_line.set_data(
                [current_distance, current_distance], [min(electronic_energies) - 0.1, max(nuclear_energies) + 0.1]
            )
        ...
        anim = animation.FuncAnimation(
            fig, animate, init_func=init, frames=2 * len(distances) - 2, interval=100, blit=False
        )
        ...
        try:
            ...
            static_ax1.text(
                0, -1.7, f"Bond Distance: {eq_distance:.2f} Å", fontsize=12, ha="center", color="green", weight="bold"
            )
        ...
        try:
            ...
            # Add annotation
            ax.text(min_distance + 0.1, min_energy, f"Ground State: {min_energy:.4f} Ha", fontsize=12)
...
def animate_vqe_optimization(save_path):
    ...
    try:
        ...
        ax1.axhline(y=ground_state_energy, color="g", linestyle="--", alpha=0.7, label="True Ground State")
        ...
        iter_text = ax1.text(0.5, 0.05, "", transform=ax1.transAxes, bbox=dict(facecolor="white", alpha=0.8))
        state_text = ax2.text(0.05, 0.05, "", transform=ax2.transAxes, bbox=dict(facecolor="white", alpha=0.8))
        ...
        def animate(i):
            if i < len(thetas):
                ...
                state_text.set_text(
                    f"|ψ⟩ = {amp_0101:.4f}|0101⟩ + {amp_1010:.4f}|1010⟩\n"
        ...
        # Create animation
        anim = animation.FuncAnimation(fig, animate, init_func=init, frames=num_iterations, interval=500, blit=False)
...
def create_ground_state_energy_visualization(save_path):
    ...
    # Add annotations
    ax.annotate(
        f"Ground State Energy: {min_energy:.4f} Ha",
    ...
    textstr = "\n".join(
        (
            ...
            f"Bond Distance: {min_distance:.2f} \u00c5",
            f"Electronic Energy: {electronic_energies[min_idx]:.4f} Ha",
            f"Nuclear Repulsion: {nuclear_energies[min_idx]:.4f} Ha",
            f"Total Energy: {min_energy:.4f} Ha",
        )
    )
...
def create_vqe_steps_visualization(save_path):
    ...
    ax1.text(
        1,
        4,
        r"$\hat{H}_{elec} = \sum_{pq} h_{pq} a_p^\dagger a_q"
        r" + \frac{1}{2}\sum_{pqrs} h_{pqrs} a_p^\dagger a_q^\dagger a_r a_s$",
        fontsize=12,
    )
    ...
    ax1.text(
        1,
        0.5,
        r"Example: $c_1 \hat{Z}_0\hat{Z}_1 + c_2 \hat{X}_0\hat{X}_1\hat{Z}_2 + c_3 \hat{I}_0\hat{Y}_1\hat{Y}_2 + ...$",
        fontsize=10,
    )
    ...
    ax3.text(
        1,
        4.3,
        r"$E(\theta) = \langle\psi(\theta)|\hat{H}_{qubit}|\psi(\theta)\rangle"
        r" = \sum_i c_i \langle\psi(\theta)|\hat{P}_i|\psi(\theta)\rangle$",
        fontsize=12,
    )
...
def create_molecular_orbital_visualization(save_path):
    ...
    ax.text(0, -1.7, "Lower Energy State", fontsize=12, ha="center", bbox=dict(facecolor="white", alpha=0.8))
    ...
    ax.text(0, -1.7, "Higher Energy State", fontsize=12, ha="center", bbox=dict(facecolor="white", alpha=0.8))
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            vqe_energy_curves.create_vqe_energy_curves_visualization(os.path.join(output_dir, "vqe_energy_curves.png"))
